...
- 会話履歴を `st.session_state` で管理し、画面に表示するとともに LLM にも渡すため、
  ...
  会話の途中で別の専門家を選ぶと警告が表示され、そのまま送信すると会話履歴はリセットされます。
- LLM 呼び出しに失敗した場合は、原因別 (API キー無効、利用上限、接続失敗など) の案内を画面に表示し、
  詳細なスタックトレースを「エラーの詳細」に折りたたみ表示します。
  同じ内容はログにも出力されるため、Streamlit Community Cloud では「Manage app」のパネルで確認できます。
